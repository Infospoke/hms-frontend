...
// ─── Short descriptions per sub-module id ─────────────────────────────────────
...
// ─── 18-entry Icon + Colour Palette ──────────────────────────────────────────
/**
 * Each entry = unique circular-icon style.
 * Assignment: ICON_PALETTE[ moduleId % ICON_PALETTE.length ]
 * This cycles through all 18 styles no matter how many modules the API returns.
 */
...
// ─── Keyword → icon override (colour still comes from palette) ────────────────
...
export class EditRoleInfoComponent implements OnInit {
  ...
  /**
   * Driven by router state `type`:
   *   'view' → read-only  (checkboxes disabled, footer shows Back only)
   *   'edit' → interactive (default)
   *
   * Navigate examples:
   *   this.router.navigate(['/role/edit'], { state: { roleId: 5, type: 'view' } });
   *   this.router.navigate(['/role/edit'], { state: { roleId: 5, type: 'edit' } });
   */
  isViewMode = false;
  ...
  /**
   * Returns the full palette entry for a given moduleId.
   * Cycles through the 18-entry palette: ICON_PALETTE[moduleId % 18]
   */
  ...
  /**
   * Returns FA icon class by keyword-matching the module name.
   * Falls back to the palette icon for that moduleId slot.
   */
  ...
  /** Which parent moduleId owns a sub-module (used to pick row-icon colour) */
  ...
  onUpdate(): void {
    ...
    const payload = Array.from(this.permMap.values()).map(p => ({
      moduleId: p.moduleId,
      create  : p.create,
      view    : p.view,
      edit    : p.edit,
    ...
    from(this.svc.updatePermission({ roleId: this.roleId, permissions: payload }))
      .subscribe({
