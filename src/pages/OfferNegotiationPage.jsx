...
import { ChevronLeft, ShieldCheck, Check, Upload, FileText, X } from 'lucide-react';
import NegotiationItemRow from '../components/careers/NegotiationItemRow.jsx';
import { useAuth } from '../context/AuthContext.jsx';
import { getOfferDetailsByApplicantIdRequest, negotiateOfferRequest } from '../lib/api.js';
import { normalizeOfferDetail, buildNegotiationItems, formatCurrency } from '../data/offers.js';

const EMPTY_VALUE = { requested: '', reason: '' };

/** Strips currency formatting ("$180,000" -> 180000) so it can be sent as a number. */
...
export default function OfferNegotiationPage() {
  ...
  useEffect(() => {
    ...
    getOfferDetailsByApplicantIdRequest(token, applicantId, controller.signal)
      .then((result) => {
        setOffer(normalizeOfferDetail(result.data || {}));
      })
      .catch((err) => {
        if (err?.name === 'AbortError') return;
        setLoadError(err.message || 'Could not load this offer. Please try again.');
      })
      .finally(() => {
  ...
  const items = useMemo(() => (offer ? buildNegotiationItems(offer) : []), [offer]);
  ...
  const toggleItem = (id) => {
    ...
    setValues((prev) => (prev[id] ? prev : { ...prev, [id]: EMPTY_VALUE }));
  };
  ...
  const toggleSelectAll = () => {
    if (allSelected) {
      ...
      setValues((prev) => {
        const next = { ...prev };
        items.forEach((item) => {
          if (!next[item.id]) next[item.id] = EMPTY_VALUE;
        });
        return next;
      });
    }
  ...
  const handleFiles = (fileList) => {
    ...
    if (next.length) setFiles((prev) => [...prev, ...next].slice(0, 5));
  };
  ...
  const isItemComplete = (id) => {
    ...
    if (id === 'other') return value.reason.trim().length > 0;
    return value.requested.trim().length > 0 && value.reason.trim().length > 0;
  };

  const canSubmit =
    confirmed &&
    selectedIds.size > 0 &&
    Array.from(selectedIds).every((id) => isItemComplete(id)) &&
    !submitting;
    ...
    return (
      <main className="mx-auto max-w-3xl px-6 py-20 text-center lg:px-10">
        <p className="text-lg font-bold text-slate-900">
          {loadError ? 'Something went wrong.' : "We couldn't find that offer."}
        </p>
        <p className="mt-2 text-sm text-slate-500">
          {loadError || 'It may have been withdrawn, or the link is out of date.'}
        </p>
        <button
          type="button"
  ...
  if (submitted) {
    return (
      <main className="mx-auto flex max-w-2xl flex-col items-center px-6 py-20 text-center lg:px-10">
        ...
        <h1 className="mt-6 text-3xl font-extrabold text-slate-900">Negotiation request sent</h1>
        <p className="mt-3 text-slate-500">
          {offer.recruiter ? `${offer.recruiter} will review` : 'Our team will review'} your request and
          follow up as soon as possible.
        </p>
        <button
          type="button"
          onClick={() => navigate(`/dashboard-careers/offer/${applicantId}`)}
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 lg:px-10">
        ...
        <h1 className="text-3xl font-extrabold text-slate-900 sm:text-4xl">Offer negotiation</h1>
        <p className="mt-3 max-w-xl text-slate-500">
          Tell us what you&apos;d like to revise. Your request will be reviewed by our team.
        </p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
          <div className="space-y-6">
            <div className="rounded-2xl border border-slate-100 bg-white shadow-card">
              <div className="flex items-center justify-between px-6 pt-6 sm:px-8">
                <div>
                  <h2 className="text-lg font-bold text-slate-900">What would you like to negotiate?</h2>
              ...
              <div className="mt-5">
                {items.map((item, i) => (
                  <NegotiationItemRow
                    ...
                    value={values[item.id] ?? EMPTY_VALUE}
                    onToggle={toggleItem}
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 p-6 sm:p-8">
              <h2 className="text-base font-bold text-brand-700">Supporting documents (optional)</h2>
              <p className="mt-1 text-sm text-slate-500">Upload any documents that support your request.</p>

              <div
                onClick={() => document.getElementById('negotiation-file-input')?.click()}
                onDragOver={(e) => {
                  e.preventDefault();
                  setDragOver(true);
                }}
              ...
              >
                ...
                <p className="text-xs text-slate-500">PDF, DOC, DOCX, PNG, JPG (Max. 5 files, 10MB each)</p>
              </div>
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-card sm:p-8">
              <h2 className="text-base font-bold text-slate-900">Overall justification (optional)</h2>
              <p className="mt-1 text-sm text-slate-500">Help us understand your overall situation.</p>
              <textarea
                rows={4}
                maxLength={1000}
                value={justification}
              ...
              />
              <p className="mt-1 text-right text-xs text-slate-400">{justification.length} / 1000</p>
            </div>
            ...
            <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {submitting ? 'Sending…' : 'Submit negotiation'}
                </button>
              </div>
            </div>
          </div>
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-card">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Offer summary</p>
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-slate-100 bg-white p-5 shadow-card">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900">Your request is important</p>
                <p className="mt-1 text-sm text-slate-500">
