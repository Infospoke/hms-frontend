...
const API_BASE_URL_AI =
  (typeof import.meta !== 'undefined' && import.meta.env?.VITE_API_BASE_URL_AI) ||
  'http://172.16.1.101:5002';


export class ApiError extends Error {
  constructor(message, { responsecode, status, errors } = {}) {
...
export function logoutRequest(token) {
  return request('/hms/candidate/logout', {
    token
  });
}
...
export function forgotPasswordRequest(email) {
  return request('/hms/candidate/forgot-password', {
    body: {
      email
    }
  });
...
export function getCandidateOffersRequest(token, signal) {
  return request('/hms/candidate/candidate-offers', {
    method: 'GET',
    token,
    signal
  });
...
export function getOfferDetailsByApplicantIdRequest(
  token,
  applicantId,
  signal
) {
  return request(
    `/hms/offer-details/get-offer-details-by-applicant-id/${applicantId}`,
    {
      method: 'GET',
      token,
      signal
...
const OFFER_LETTER_PATH =
  '/hms/offer-details/download/offerLetter';


export function offerLetterUrl(
  applicantId,
  action = 'view'
) {
...
export async function fetchOfferLetterBlob(
  token,
  applicantId,
  action,
  signal
  ...
  try {
    ...
    response = await fetch(
      offerLetterUrl(applicantId, action),
      {
        headers: {
          'X-Channel': 'web',
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      `Could not load the offer letter (${response.status}).`,
      {
        status: response.status
      }
    );
...
export function getCandidateInterviewsRequest(
  token,
  signal
) {
  return request('/hms/candidate/interviews', {
    method: 'GET',
    token,
    signal
  });
...
export function getMyApplicationsRequest(
  ...
  return request('/hms/candidate/get-my-applications', {
    method: 'GET',
    token,
    signal
  });
...
export function getJobsList(
  ...
  return request('/hms/create-job/get-all-jobs', {
    token,
    body: payload,
    signal
  });
...
export function getJobDetailsByIdRequest(
  token,
  jobId,
  signal
) {
  return request(
    `/hms/create-job/get-job-details/${jobId}`,
    {
      method: 'GET',
      token,
      signal
...
export function getCandidateDetailsRequest(
  token,
  candidateId,
  signal
) {
  return request(
    `/hms/candidate/details/${candidateId}`,
    {
      method: 'GET',
      token,
      signal
...
export function applyJobRequest(
  token,
  {
    candidateId,
    jobId,
...
export function analyzeResumeRequest(
  ...
  return request(
    '/api/resume/analyze/batch',
    {
      ...
      baseUrl:
        'https://2g7634mr-5002.inc1.devtunnels.ms'
    }
  );
}
...
export function resumeReuploadRequest(
  token,
  {
    applicationId,
    updateProfileResume,
...
export function createCandidateRequest({
  ...
  if (additionalFile) {
    form.append('additionalFile', additionalFile);
  }
