...
export default function ApplicationFormPage() {
  ...
  // state instead of calling get-job-details again.
  ...
  useEffect(() => {
    ...
    getJobDetailsByIdRequest(token, jobId, controller.signal)
      ...
      .catch((err) => {
        ...
        setJobError(
          err.message || 'Could not load this role. Please try again.'
        );
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    getCandidateDetailsRequest(
      ...
      .then((result) => {
        setCandidate(
          normalizeCandidateDetail(result.data || {})
        );
      })
      .catch((err) => {
  ...
  const [resumeChoice, setResumeChoice] = useState('on-file');
  ...
  // If no resume exists, select upload option.
  useEffect(() => {

    if (!candidateLoading && !hasResumeOnFile) {
      setResumeChoice('new');
    }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (seconds < 5) {
        ...
        setSavedLabel(`Saved ${seconds}s ago`);
      }
      else {
        setSavedLabel(`Saved ${Math.round(seconds / 60)}m ago`);
      }
  ...
  const canApply = useMemo(() => {
    ...
    if (!consent || candidateLoading || !candidate) {
      return false;
    }

    if (resumeChoice === 'new' && !newResume) {
      return false;
    }

    if (resumeChoice === 'on-file' && !hasResumeOnFile) {
      return false;
    }
  ...
  const handleBackToRole = () => {
    navigate(`/dashboard-careers/${jobId}`);
  };
const handleApply = async () => {
  ...
  try {
    ...
    const applyResponse = await applyJobRequest(token, {
      candidateId: candidate.candidateId,
      jobId,
      resumeFile: resumeChoice === 'new' ? newResume : undefined,
    });

    // Application ID returned by Apply Job API
    const applicationId = applyResponse.data;

    console.log('Apply Job Response:', applyResponse);
    console.log('Application ID:', applicationId);

    
    analyzeResumeRequest(token, {
      batch_id: 1,
      resume_batch: [applicationId],
    }).catch((error) => {
      console.error('Resume analysis failed:', error);
    });


    console.log('Apply Job Message:', applyResponse.message);

    // Navigate after Apply Job succeeds
    navigate('/dashboard-careers/applications');

  } catch (err) {

    // ONLY Apply Job API errors come here
    setSubmitError(
      err.message ||
      ...
      <main className="mx-auto max-w-3xl px-6 py-10 lg:px-10">
        ...
        <div className="mt-8 rounded-2xl border border-slate-100 bg-white p-6 shadow-card sm:p-8">
                ...
                <span className="flex items-center gap-3">
                  ...
                  <span>
                    ...
                    <span className="block text-xs text-slate-500">
                      {hasResumeOnFile
                        ? 'Already on file'
                        : 'Upload a resume below to apply'
                      }
                    </span>
          ...
          <section>
            ...
            <p className="mt-1.5 text-sm text-slate-500">
              One thing to confirm before you apply.
            </p>
                ...
                label={
                  <span>
                    ...
                    <span className="font-semibold text-slate-800">
                      I consent to Nexus processing my application data{' '}
